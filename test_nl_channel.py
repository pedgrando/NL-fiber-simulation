...
from parameters import (
    beta2_norm, gamma_norm, sigma_sq, Bn, P_norm, power_scale_norm, M, P
)
...
def main():
    ...
    q0t = (A1*np.exp(1j*omega0*t) + A2*np.exp(-1j*omega0*t))*np.exp(-(t**2)/2*(D**2)) 
    ...
    qzt, qzft = nl_channel(t, q0t, z = L, Nsteps = Nsteps, gamma = gamma, sigma2 = 0, betav = betav, f = f, B = B)
    ...
    axes[1].plot(np.fft.fftshift(f), np.abs(np.fft.fftshift(Q0f))**2, '--', label = '|q̂(ω, 0)|²')
    axes[1].plot(np.fft.fftshift(f), np.abs(np.fft.fftshift(qzft))**2, label = '|q̂(ω, L)|² (before eq.)')
    axes[1].set_title('|q̂(ω, z)|² (before eq.)')
    axes[1].set_xlabel('f (normalized)')
    axes[1].set_xlim([-7*B, 7*B])
    axes[1].grid(True)
    plt.tight_layout()
    ...
    axes[1].plot(np.fft.fftshift(f), np.abs(np.fft.fftshift(Q0f))**2, label = '|q̂(ω, 0)|²')
    axes[1].plot(np.fft.fftshift(f), np.abs(np.fft.fftshift(qzf_eq))**2, '--', label='|q̂(ω, L)|² (eq.)')
    axes[1].set_title('Equalized vs transmitted (freq)')
    axes[1].set_xlabel('f (normalized)')
    axes[1].set_xlim([-3*B, 3*B])
    axes[1].legend()
    axes[1].grid(True)
